...
#' 
autodock_vina <- function(prot_pdb, ligand_pdb, 
    ...
    mgltools_utils_dir <- file.path(mgltools_dir, "MGLToolsPckgs/AutoDockTools/Utilities24")
    prepare_receptor <- file.path(mgltools_utils_dir, "prepare_receptor4.py")
    prepare_ligand <- file.path(mgltools_utils_dir, "prepare_ligand4.py")
    pythonsh_bin <- file.path(mgltools_dir, "bin", "pythonsh") # 使用pythonsh

    # 定义Vina可执行文件的路径
    vina_exe <- file.path(autodock_vina_dir, "bin", "vina")
    ...
    receptor_status <- system(receptor_cmd, ignore.stdout = TRUE, ignore.stderr = FALSE)
    ...
    ligand_status <- system(ligand_cmd, ignore.stdout = TRUE, ignore.stderr = FALSE)
    if (ligand_status != 0) {
    ...
    if (is.null(center)) {
        ...
        con <- file(prot_pdb, "r")
        coords <- list(x = numeric(), y = numeric(), z = numeric())
        while (length(line <- readLines(con, n = 1)) > 0) {
            if (startsWith(line, "ATOM") || startsWith(line, "HETATM")) {
                ...
                if (grepl("^[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?$", x_str) &&
                  ...
                  x_val <- as.numeric(x_str)
                  y_val <- as.numeric(y_str)
                  z_val <- as.numeric(z_str)
    ...
    vina_status <- system(vina_cmd, ignore.stdout = FALSE, ignore.stderr = FALSE)
    if (vina_status != 0) {
